...
class Solution:
    def gameOfLife(self, board):
        ...
        def record_change(ri, ci):
            live_num = 0
            start_row = max(ri - 1, 0)
            start_column = max(ci - 1, 0)
            end_row = min(ri + 1, row_num - 1)
            end_column = min(ci + 1, column_num - 1)
            for i in range(start_row, end_row + 1):
                for j in range(start_column, end_column + 1):
                    if i != ri or j != ci:
                        live_num = live_num + board[i][j]
            if board[ri][ci] == 1 and live_num < 2 or live_num > 3:
                to_change[ri, ci] = 0
            if board[ri][ci] == 0 and live_num == 3:
                to_change[ri, ci] = 1
        ...
        for (ri, ci) in to_change.keys():
            board[ri][ci] = to_change[ri, ci]
